...
#define TAG "ti.playservices"

using namespace v8;

static Persistent<Object> bindingCache;

static void PlayServices_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "PlayServices.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::PlayServicesBindings::lookupGeneratedInit(
...
static void PlayServices_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, PlayServices_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void PlayServices_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::PlayServicesBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry PlayServicesBinding = {
	"ti.playservices",
	PlayServices_init,
	PlayServices_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_playservices_PlayServicesBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.playservices", &PlayServicesBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::PlayServicesBindings::lookupGeneratedInit));
}
